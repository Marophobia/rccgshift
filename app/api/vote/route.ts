        ...
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (!user || !user.user_sessions) {
                    throw new Error('User not found or no voting session available.');
                }
                ...
                if (!userCurrentSession) {
                    throw new Error('User session for the current round not found.');
                }
            ...
            return errorHandler(`Unable to update vote: ${transactionError}`, 500);
        }
    } catch (error) {
