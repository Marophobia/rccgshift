...
const CoordintorsSingleForm = ({ official, settings }: { official: IOfficials, settings: Isettings }) => {
    ...
    const [amount, setAmount] = useState(official.official_sessions[0].amount_to_pay - official.official_sessions[0].amount_paid);
    ...
    return (
        <>
            ...
            <>
                ...
                {!official.official_sessions[0].status && official.official_sessions[0].amount_paid < official.official_sessions[0].amount_to_pay && (
                    //payment incomlete
                    <>
                        <p>
                            You have not paid the required contribution fee. Please click on the button below to pay the
                            required contribution fee which is <span className='text-white'>₦{formattedAmount}</span>
                        </p>
                        <div>
                ...
                {official.official_sessions[0].status && official.official_sessions[0].amount_paid >= official.official_sessions[0].amount_to_pay && (
                    <>
                        <p>
                            You have already paid the required contribution fee. Thank you for your payment.
                        </p>
                    </>
                )}
            </>
