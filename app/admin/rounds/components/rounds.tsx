...
const RoundsCard = (props: Props) => {
    ...
    const continuer = async () => {
        ...
        if (qualifiers >= contestants) return toast.error('Qualifiers cannot be equal to or greater than the contestants')
            ...
            <AlertDialog>
                <AlertDialogTrigger>
                    <div className="mb-4 w-full d-flex justify-center">
                        <button className="btn b-light btn-primary-light mt-3">Start New Round</button>
                    </div>
                </AlertDialogTrigger>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            <div>
                                <label className="form-label">Input number of Qualifiers</label>
                                <input type="number" value={qualifiers} onChange={handleChange} className="form-input" autoComplete="off" required />
                            </div>
                            <p className="text-red-600">Please Note that this action cannot be undone</p>
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={continuer}>Continue</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog >
                        ...
                        <div className="col-span-3 mr-2" key={round.id}>
                            <div className="card">
                                <div className="card-body ">
                                    ...
                                    <div className="py-10">
                                        <h1 className="text-center" style={{ fontWeight: "700" }}>{round.name}</h1>
                                    </div>
                                    <div className="flex gap-2 justify-center">
                                        ...
                                        <div className="btn b-outline btn-info-outline">
                                            Qualifiers: {index + 1 < rounds.length ? rounds[index + 1].users.length : round.users.length}
                                        </div>
                                    </div>
                                    <div className="w-full flex justify-center">
                                        <Link href={`/admin/rounds/${round.id}`} className="btn b-solid btn-primary-solid mt-3 w-full">More Details</Link>
                                    </div>
